...
import IphoneDuoFrame from './iphone-duo-frame';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    let frame = 0, smoothHero = 0, smoothPortrait = 0, previousTime = 0;
    const update = (now: number) => {
      ...
      const portraitRect = portrait.current?.getBoundingClientRect();
      const portraitTarget = active && portraitRect ? Math.max(-10, Math.min(10, (innerHeight / 2 - portraitRect.top - portraitRect.height / 2) / innerHeight * 20)) : 0;
      smoothPortrait += (portraitTarget - smoothPortrait) * easing;
      portrait.current?.style.setProperty('--portrait-y', `${smoothPortrait}px`);
      cards.forEach(card => {
      ...
      if (Math.abs(heroTarget - smoothHero) > .0001 || Math.abs(portraitTarget - smoothPortrait) > .01) frame = requestAnimationFrame(update);
    };
    ...
    <main>
      ...
      <section id="journal" className="journal section-wrap">
        ...
        <div className="post-grid project-grid">{posts.map((post, index) => <article className="post-shell" key={post.title} data-reveal style={{ transitionDelay: `${index % 2 * 90}ms` }}>
          <button className={`post-card project-card ${post.style}`} onPointerMove={tilt} onPointerLeave={event => { event.currentTarget.style.setProperty('--rx', '0deg'); event.currentTarget.style.setProperty('--ry', '0deg'); }} onClick={event => { readerTrigger.current = event.currentTarget; setSelected(index); setReaderOpen(true); }} aria-label={`阅读项目：${post.title}`}>
            ...
            <div className="project-art">{index === 0 ? <div className="product-phone-pair"><DeviceFrame src="./images/tf-rolls.webp" alt="TravelFilm 旅行胶卷界面"/><DeviceFrame src="./images/tf-detail.webp" alt="TravelFilm 行程详情界面"/></div> : <IphoneDuoFrame src={post.image} alt={post.alt}/>}</div>
      ...
      <section id="experience" className="experience-section">
        ...
        {experience.map((item,index) => <article key={item.company} className={`experience-item experience-${index}`} data-reveal>
          <div className="experience-banner">{index===0 && <div className="loopit-real-covers"><div className="case-glass"><img src="./images/loopit-case-01.webp" alt="案例 01：MyJJKDomain 三卡策略游戏" loading="lazy"/></div><div className="case-glass"><img src="./images/loopit-case-04.webp" alt="案例 04：Emergency Draft 三英雄策略游戏" loading="lazy"/></div><div className="case-glass"><img src="./images/loopit-case-06.webp" alt="案例 06：Spider Sense Online 节奏点击互动" loading="lazy"/></div></div>}<div className="experience-banner-shade"/><div className="company-heading"><div className="eyebrow">{item.date}</div><h3>{item.company}<span>{item.role}</span></h3><p>{item.headline}</p><span className="company-fullname">{item.fullName}</span></div>{index===1&&<div className="baidu-visual"><a className="baidu-browser-preview baidu-shot-poem" href="https://hanyu.baidu.com/shici/detail?pid=9c26d930bfbc48778c9a46a8364b4f96" target="_blank" rel="noreferrer" aria-label="查看百度汉语诗词详情 · 小池"><img src="./images/baidu-hanyu-poem.webp" alt="百度汉语当前公开页面：诗词详情 · 小池" width="1400" height="875" loading="lazy" decoding="async"/></a><a className="baidu-browser-preview baidu-shot-real" href="https://hanyu.baidu.com/hanyu-page/zici/s?wd=%E5%B1%B1&amp;ptype=zici" target="_blank" rel="noreferrer" aria-label="查看百度汉语汉字释义 · 山"><img src="./images/baidu-hanyu-real.webp" alt="百度汉语当前公开页面：汉字释义 · 山" width="1400" height="875" loading="lazy" decoding="async"/></a><a className="baidu-browser-preview baidu-shot-home" href="https://hanyu.baidu.com/" target="_blank" rel="noreferrer" aria-label="查看百度汉语搜索首页"><img src="./images/baidu-hanyu-home.webp" alt="百度汉语当前公开页面：搜索首页" width="1400" height="875" loading="lazy" decoding="async"/></a></div>}</div>
      ...
      <section id="about" className="about section-wrap">
        ...
        <div className="about-grid"><div data-reveal><h2>研究问题。<br/>设计体验。<br/><span>也亲手把它做出来。</span></h2><div className="about-directions"><span>AI 应用</span><span>Agent 工作流</span><span>内容与效率产品</span></div><PortraitGallery galleryRef={portrait} paused={paused}/></div><div className="about-copy" data-reveal><p>天津大学环境科学硕士在读，入选首批「科创硕士」培养项目。曾在 <strong>Loopit 与百度</strong>担任产品经理实习生，<strong>独立设计、开发和运营</strong>多个产品。</p><p className="muted">从研究中训练问题定义与证据意识，在产品实践中连接用户需求、交互设计、数据分析与上线交付。</p><div className="skill-lines"><div><span>产品与交付</span><p>用户 / 竞品研究、需求拆解、PRD、流程与高保真原型、上线验收与复盘</p></div><div><span>AI 与数据</span><p>LLM / Agent 场景设计、真实任务评测、Badcase 归因、Python / SQL、漏斗与留存分析</p></div><div><span>内容与观察</span><p>小红书个人账号 1K+、救助主题账号 4K+；选题策划、内容发布与用户反馈迭代</p></div></div></div></div>

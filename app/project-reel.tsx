...
const moments = [
  ...
  { label:'代表性 Case', eyebrow:'MADE TO BE PLAYED', title:<>把好想法，<br/>交给你来玩。</>, description:'六个互动原型，两组世界模型探索。让策略、节奏与创意，变成可体验的作品。', image:'', alt:'Loopit 互动案例', url:'https://awoele.github.io/yxy-loopit-cases/', style:'work-cases' },
    ...
    <div className="reel-stage">
      ...
      <div className="reel-viewport"><div className="reel-track" ref={railRef}>
        {moments.map((moment,i)=><article key={moment.label} className={`reel-panel loopit-work-panel ${moment.style}`}>
          ...
          <div className="work-art">{i===2 ? <div className="work-case-fan">{['01','04','06'].map(n=><figure key={n}><img src={`/images/loopit-case-${n}.webp`} alt={`Loopit 案例 ${n} 真实试玩画面`} loading="lazy" width="450" height="694"/></figure>)}</div> : <a className="work-screens" href={moment.url} target="_blank" rel="noreferrer" aria-label={`打开${moment.label}`}><figure className="work-window work-window-main"><img src={moment.image} alt={moment.alt} loading="lazy" width="1400" height="875"/></figure><figure className="work-window work-window-detail"><img src={`/images/loopit-work-${i}-detail.webp`} alt={`${moment.label}的另一处真实页面内容`} loading="lazy" width="1400" height="875"/></figure></a>}</div>
        </article>)}
      </div></div>
